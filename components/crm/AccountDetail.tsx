        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: 0, fontFamily: 'DM Serif Display, serif' }}>
                {account.name}
              </h1>
              <span style={{
                fontSize: 11, fontWeight: 700,
                color: SEGMENT_COLOR[account.segment],
                background: SEGMENT_COLOR[account.segment] + '18',
              ...
              {critTasks > 0 && (
                <span style={{
                  ...
                  background: 'var(--red-dim)', borderRadius: 99, padding: '2px 8px',
                }}>
                  {critTasks} critical task{critTasks !== 1 ? 's' : ''}
                </span>
              )}
            </div>
            <div style={{ fontSize: 13, color: 'var(--text3)' }}>
              {account.industry} · {account.location} · Owner: <span style={{ color: 'var(--text2)' }}>{account.owner}</span>
            </div>
          </div>
          {account.website && (
            <a
              ...
              style={{ fontSize: 12, color: 'var(--blue)', textDecoration: 'none', padding: '6px 12px', border: '1px solid var(--border2)', borderRadius: 'var(--r-sm)' }}
            >
              {account.website} ↗
            </a>
          )}
